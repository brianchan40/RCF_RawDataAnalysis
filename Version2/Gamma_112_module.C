...
void Gamma_112_module(int cen = 1, int opt_weight = 1, const TString JobIDName = "1234")      //main_function
{
    Run     = details->Run;
    ...
    if(!IsGoodEvent(cen)) return;

    if(debug_1) std::cout << "debug_1 1" << endl;

    if(!CountCharge()) return;;
    ...
    if(debug_1 && (n_Lambdas != 0)) std::cout << "n_Lambdas 1 = " << n_Lambdas << endl;

    Phi_new.resize(n_Lambdas);
    for(int trki = 0; trki < n_Lambdas; trki++)
    {
        gv_gamma::trk_mom->SetXYZ(p_lambda->at(trki).px, p_lambda->at(trki).py, p_lambda->at(trki).pz);
        Eta   = gv_gamma::trk_mom->Eta();
        Pt    = gv_gamma::trk_mom->Pt();
        Charge = p_lambda->at(trki).Charge; //1 for lambda, -1 for antilambdas
    ...
    if(debug_1 && (n_protons != 0)) std::cout << "n_protons 1 = " << n_protons << endl;

    Phi_pnew.resize(n_protons);
    for(int trki = 0; trki < n_protons; trki++)
    {
        //if(p_proton->at(trki).prim != 1) continue;
        gv_gamma::trk_mom->SetXYZ(p_proton->at(trki).px, p_proton->at(trki).py, p_proton->at(trki).pz);
        Eta2   = gv_gamma::trk_mom->Eta();
        Pt2   = gv_gamma::trk_mom->Pt();
        Charge2 = p_proton->at(trki).Charge;
        //DCAGlobal = p_all[trk].DCAGlobal();
    ...
    for(int trki = 0; trki < n_Lambdas; trki++)
    {
        ...
        for(int trkj = 0; trkj < n_protons; trkj++)
        {
            //if(p_proton->at(trkj).prim != 1) continue;
            Charge2 = p_proton->at(trkj).Charge; //1 for protons, -1 for antiprotons
            gv_gamma::trk_mom2->SetXYZ(p_proton->at(trkj).px, p_proton->at(trkj).py, p_proton->at(trkj).pz);
            Eta2   = gv_gamma::trk_mom2->Eta();
            Pt2    = gv_gamma::trk_mom2->Pt();
            DCAGlobal2 = p_proton->at(trkj).dcaglobal;
