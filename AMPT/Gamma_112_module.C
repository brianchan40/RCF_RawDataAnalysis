...
void Gamma_112_module(int cen = 1, int opt_weight = 1, const Char_t *inFile = "./st_physics_12160020_raw_1010001.picoDst.root", const TString JobIDName = "1234", const TString lam_type = "lam")      //main_function
{
    set_debug();
    ...
    for(int i = 0; i < nentries; i++)
    {
        ...
        if(debug_1) std::cout << "i = " << i << ", nentries = " << nentries << endl;
        ...
        std::fstream myfile("./centrality.txt", std::ios_base::in);
        ...
        while (myfile >> a)
        {
            if(a > imp)
            {
                if(count < 2) Centrality = (8 - count);
                else if(count < 16) Centrality = (7 - count / 2);
                else Centrality = -1;

                if(debug_1) cout << "imp = " << imp << ", Centrality = " << Centrality << endl;
                break;
            }
            count++;
        }
        ...
        if(Centrality != cen) continue;
        if(debug_1) cout << "Centrality = " << cen << endl;
        if((npp + npt) < 3) continue;
        if(debug_1) cout << "(npp + npt) = " << (npp + npt) << endl;

        //Eweight = details->Eweight;
        Eweight = 1;

        if(debug_1) cout << "particle list?" << endl;
        determine_particlelists(lam_type);

        if(debug_1) cout << "nope not particle list?" << endl;
        ...
        for(int trki = 0; trki < n_particle1; trki++)
        {
            ...
            if(debug_2) std::cout << "Eta = " << Eta << endl;

            //if(!IsGoodPOI(Pt, Eta, DCAGlobal)) continue;
            

            if(debug_2) std::cout << "debug_1 5" << endl;

            // FillPhiPOI();       //Charge is needed here

            if(debug_2) std::cout << "debug_1 6" << endl;
        ...
        {
            if(debug_1) std::cout << "Beginning of Loop 2" << endl;
            gv_gamma::trk_mom->SetXYZ(particle2->at(trki).px, particle2->at(trki).py, particle2->at(trki).pz);
            if(debug_1) std::cout << "trki = " << trki << ", n_particle2 = " << n_particle2 << ", particle2->size() = " << particle2->size() << endl;
            Pt2   = gv_gamma::trk_mom->Pt();
            if(debug_1) std::cout << "Pt2 = " << Pt2 << endl;
            //if((Pt2 < 0.2) || (gv_gamma::trk_mom->Mag() > 2.0)) continue;
        ...
        for(int trki = 0; trki < n_particle1; trki++)
        {
            if(debug_1) std::cout << "Beginning of Real Analysis Loop" << endl;
            //if((particle_option1 == 0) && ((particle1->at(trki).mass < 1.113) || (particle1->at(trki).mass > 1.119))) continue;
            gv_gamma::trk_mom->SetXYZ(particle1->at(trki).px, particle1->at(trki).py, particle1->at(trki).pz);
            if(debug_1) std::cout << "trki = " << trki << ", n_particle1 = " << n_particle1 << ", particle1->size() = " << particle1->size() << endl;
            Pt    = gv_gamma::trk_mom->Pt();
            if(debug_1) std::cout << "Pt = " << Pt << endl;
            //if(Pt > 2.8 || Pt < 0.4) continue;
            if((Pt < 0.5) || (Pt > 2.0)) continue;
            Eta   = gv_gamma::trk_mom->Eta();
            if(debug_1) std::cout << "Eta = " << Eta << endl;
            Charge = particle1->at(trki).Charge; //1 for lambda, -1 for antilambdas
            if(debug_1) cout << "Lambda charge = " << Charge << endl;
            //DCAGlobal = p_all[trk].DCAGlobal();
            Phi   = gv_gamma::trk_mom->Phi();
            ...
            if(debug_1) cout << "Hit an EP proton! in Lambda..." << endl;
            //if(IsGoodAsso(Pt, Eta, DCAGlobal))
            //{
            // mQx_i -= Pt * cos(Phi_new[particle1->at(trki).trk_id] * nHar);
            // mQy_i -= Pt * sin(Phi_new[particle1->at(trki).trk_id] * nHar);
            //}
            ...
            for(int trkj = 0; trkj < n_particle2; trkj++)
            {
                ...
                if(debug_1) cout << "Hit an EP proton!" << endl;
                if(debug_1) cout << "trkj = " << trkj << endl;
                if(debug_1) cout << "particle2 size = " << particle2->size() << endl;
                if(debug_1) cout << "particle2->at(trkj).trk_id = " << particle2->at(trkj).trk_id << endl;
                if(debug_1) cout << "Phi_pnew size = " << Phi_pnew.size() << endl;

                // mQx_j -= Pt2 * cos(Phi_pnew[trkj] * nHar);
                // mQy_j -= Pt2 * sin(Phi_pnew[trkj] * nHar);

                if(debug_1) cout << "maybe particle in proton loop went past?" << endl;
                ...
                if(debug_1) cout << "Gamma112 = (correlator4) " << correlator4 << endl;
                if(debug_1) cout << "Gamma112 = (correlator4e) " << correlator4e << endl;
                if(debug_1) cout << "Gamma112 = (correlator4w) " << correlator4w << endl;
        ...
        if(opt_weight != 1)
        {
            for(int trkj = 0; trkj < n_particle2; trkj++)
            {
                ...
                //{
                // mQx_j -= Pt2 * cos(Phi_pnew[particle2->at(trkj).trk_id] * nHar);
                // mQy_j -= Pt2 * sin(Phi_pnew[particle2->at(trkj).trk_id] * nHar);
                mQx_j -= Pt2 * cos(Phi_pnew[trkj] * nHar);
                mQy_j -= Pt2 * sin(Phi_pnew[trkj] * nHar);
                //}
        ...
        if(debug_1) std::cout << "Half way through" << endl;
        pTemp_parity_e_noHBT->Reset();
        pTemp_parity_w->Reset();
        pTemp_parity_w_noHBT->Reset();
        pTemp_delta->Reset();
        pTemp_delta_noHBT->Reset();
        if(debug_1) std::cout << "Done Done" << endl;

    } // Event

    if(debug_1) std::cout << "Out of event loop" << endl;
    ...
    if(opt_weight == 1)
    {
        if(debug_1) std::cout << "Time to write weights" << endl;
...
bool CountCharge()
{
    ...
    for(int trk = 0; trk < NPTracks; trk++)
    {
        ...
        Hist_DCA->Fill(0);
        if(!IsGoodAsso(PtAsso, EtaAsso, 0, p_all->at(trk).trk_id)) continue;
        Fcount++;
    }
...
bool MakeTPC_EP()
{
    ...
    for(int trk = 0; trk < NPTracks; trk++)
    {
        ...
        if(!IsGoodAsso(PtAsso, EtaAsso, DCAGlobalAsso, p_all->at(trk).trk_id)) continue;
    ...
    // if(debug_2) cout << "mQx1 = " << mQx1 << ", mQx2 = " << mQx2 << ", mQx3 = " << mQx3 << ", mQx4 = " << mQx4 << endl;
    // if(debug_2) cout << "mQy1 = " << mQy1 << ", mQy2 = " << mQy2 << ", mQy3 = " << mQy3 << ", mQy4 = " << mQy4 << endl;

    if(mQx1 == 0 || mQy1 == 0 || mQx2 == 0 || mQy2 == 0) return false;
    if(mQx3 == 0 || mQy3 == 0 || mQx4 == 0 || mQy4 == 0) return false;
...
void FillGamma(int ord)
{
    ...
    // pParity_int_obs3_splitpt[(int)floor((Pt-0.5)/0.1)];
...
void determine_particlelists(TString lam_type)
{

    if(particle1->size() != 0) particle1->clear();
    if(particle2->size() != 0) particle2->clear();
    if(p_all->size() != 0) p_all->clear();

    if(debug_1) cout << "done clearing particle list?" << endl;

    int trk_id_lam = 0;
    int trk_id_p = 0;

    for(int part1 = 0 ; part1 < (int)px->size() ; part1++)
    // for(int part1 = 0 ; part1 < refmult ; part1++)
    {
        // if(debug_1) cout << "part1 = " << part1 << ", (int)px->size() = " << (int)px->size() << endl;
        particle temp_particle;
        temp_particle.px = px->at(part1);
        temp_particle.py = py->at(part1);
        temp_particle.pz = pz->at(part1);
        // temp_particle.px = px[part1];
        // temp_particle.py = py[part1];
        // temp_particle.pz = pz[part1];
        // if(debug_1) cout << "px->at(part1) = " << px->at(part1) << ", py->at(part1) = " << py->at(part1) << endl;
        if(debug_1) cout << "man is it pinfo = " << part1 << endl;
        p_info = db->GetParticle((int)pid->at(part1));
        // p_info = db->GetParticle((int)pid[part1]);
        if(abs((int)pid->at(part1)) == 42) continue;
        // if(abs((int)pid[part1]) == 42) continue;
        // if(debug_1) cout << "(int)pid->at(part1) = " << (int)pid->at(part1) << endl;
        temp_particle.Charge = p_info->Charge() / 3;
        temp_particle.mass = p_info->Mass();

        if(particle_option1 == 0)  // first track is LAMBDA
        {
            if((pid->at(part1) == 3122) && (lam_type == "lam"))
            // if(((int)pid[part1] == 3122) && (lam_type == "lam"))
            {
                if(debug_1) cout << "Lambda track" << endl;
                temp_particle.Charge = 1;
                temp_particle.trk_id = trk_id_lam;
                particle1->push_back(temp_particle);
                trk_id_lam++;
            }
            else if((pid->at(part1) == -3122) && (lam_type == "antilam"))
            // else if(((int)pid[part1] == -3122) && (lam_type == "antilam"))
            {
                if(debug_1) cout << "ANTILambda track" << endl;
                temp_particle.Charge = -1;
                temp_particle.trk_id = trk_id_lam;
                particle1->push_back(temp_particle);
                trk_id_lam++;
        ...
        else if(particle_option1 == 1)
        {
            ...
            if(fabs(pid->at(part1)) == 2212)
            // if(abs((int)pid[part1]) == 2212)
            {
                temp_particle.trk_id = trk_id_lam;
                particle1->push_back(temp_particle);
                trk_id_lam++;
        ...
        {
            if(debug_1) cout << "Proton track?" << endl;

            if(fabs(pid->at(part1)) == 2212)
            // if(abs((int)pid[part1]) == 2212)
            {
                temp_particle.trk_id = trk_id_p;
                particle2->push_back(temp_particle);
                trk_id_p++;
            }
        }

        if(debug_1) cout << "Or P_all?" << endl;

        // temp_particle.trk_id = pid[part1];
        temp_particle.trk_id = pid->at(part1);
        p_all->push_back(temp_particle);
    }
}
