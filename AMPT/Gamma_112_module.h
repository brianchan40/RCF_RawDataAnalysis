...
const float EtaCut = 5.0;
const float DcaCut = 2;
const float Eta_EP_Cut = 0.025;  //particles in the event plane
...
TH1D *num_proton_tree = new TH1D("num_proton_tree", "Number of Protons in an Event from Tree", 20, 0, 20);
TH1D *num_lam_final = new TH1D("num_lam_final", "Number of Lambdas in an Event Actually Used", 20, 0, 20);
TH1D *num_proton_final = new TH1D("num_proton_final", "Number of Protons in an Event Actually Used", 20, 0, 20);
TH1D *num_gamma_final = new TH1D("num_gamma_final", "Number of Gamma Entries in an Event", 200, 0, 200);
...
TProfile *pParity_int_obs3_splitpt[15];
...
int   Run, Day, Day2, Day3, Trigger, RefMult, TOFMult, Centrality, NPTracks, n_particle1, n_particle2, Fcount, Scount;   //
...
float ndEdx2, nSigma_p2, nSigma_pi2, DCAGlobal2, Eta2, Theta2, Phi2, Pt2, eff2, TOFflag2, rapidity2;   //2nd track info
...
bool IsGoodAsso(float p, float e, float d, int PID);                     //cuts on EP particles
...
bool MakeTPC_EP();                        //reconstruct TPC EPs
